...
$scopeProgram = trim((string)($scope['program'] ?? ''));
$scopeDepartment = trim((string)($scope['department'] ?? ''));
...
if ($scopeProgram !== '') {
    ...
    $scopeSql = "AND u.department = ?";
    $scopeTypes = 's';
    $scopeParams = [$scopeDepartment];
    $scopeLabel = $scopeDepartment;
} elseif ($scopeCollege !== '') {
    $scopeSql = "AND u.college = ?";
    $scopeTypes = 's';
    $scopeParams = [$scopeCollege];
    $scopeLabel = $scopeCollege;
}
    ...
    if (!hash_equals($token, (string)($_POST['token'] ?? ''))) {
        ...
        if ($newStatus === '') {
            ...
            $sql = "
                ...
                WHERE u.id = ? AND u.role = 'faculty' AND u.account_status = 'pending' {$scopeSql}
            ";
            ...
            if ($stmt) {
                ...
                if ($stmt->affected_rows > 0) {
                    $message = "<div class='alert alert-success'>Faculty account updated.</div>";
                } else {
...
<head>
    <meta charset="UTF-8">
    <title>Verify Faculty Accounts</title>
...
<main class="container py-5 px-3 px-md-4" style="max-width: 1100px;">
    <div class="d-flex flex-wrap align-items-center justify-content-between mb-4">
        <div>
            <h1 class="h4 fw-semibold text-success mb-1">Faculty Verification</h1>
            <p class="text-muted mb-0">Approve or reject pending faculty registrations.</p>
        </div>
                ...
                <tbody>
                    <?php foreach ($pendingAccounts as $account): ?>
                        <tr>
                            ...
                            <td><?php echo htmlspecialchars((string)($account['specialization'] ?? '')); ?></td>
                            <td>
                                <?php
