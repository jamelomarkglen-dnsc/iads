...
function final_routing_reviewer_has_access(mysqli $conn, int $submission_id, int $reviewer_id, string $reviewer_role): bool
{
    $stmt = $conn->prepare("
        SELECT id
...
if ($action === 'create_annotation') {
    ...
    if (!final_routing_reviewer_has_access($conn, $submission_id, $user_id, $reviewerRole)) {
        http_response_code(403);
    ...
    $result = create_final_routing_annotation(
        $conn,
        $submission_id,
        $user_id,
        $reviewerRole,
    ...
    mark_final_routing_review_status($conn, $submission_id, $user_id);

    $reviewerName = trim(($_SESSION['firstname'] ?? '') . ' ' . ($_SESSION['lastname'] ?? '')) ?: 'Committee Reviewer';
    $message = "{$reviewerName} added feedback to your final routing submission.";
    notify_user(
        $conn,
        (int)$submission['student_id'],
        'Final routing feedback added',
        $message,
...
if ($action === 'fetch_annotations') {
    ...
    if ($user_role !== 'student' && !final_routing_reviewer_has_access($conn, $submission_id, $user_id, $reviewerRole)) {
        http_response_code(403);
        echo json_encode(['success' => false, 'error' => 'Unauthorized access.']);
        exit;
    }
    ...
    foreach ($annotations as &$annotation) {
        $annotation['replies'] = fetch_final_routing_annotation_replies($conn, (int)$annotation['annotation_id']);
    }
    unset($annotation);
...
if ($action === 'fetch_page_annotations') {
    ...
    $annotations = fetch_final_routing_page_annotations($conn, $submission_id, $page_number);
    foreach ($annotations as &$annotation) {
        $annotation['replies'] = fetch_final_routing_annotation_replies($conn, (int)$annotation['annotation_id']);
    }
    unset($annotation);
...
if ($action === 'update_annotation') {
    ...
    $annotation_content = trim($_POST['annotation_content'] ?? '');
    $annotation = fetch_final_routing_annotation($conn, $annotation_id);
    if (!$annotation) {
        http_response_code(404);
    ...
    $result = update_final_routing_annotation($conn, $annotation_id, $annotation_content, $user_id);
    if (!$result['success']) {
        http_response_code(400);
        echo json_encode($result);
        exit;
...
if ($action === 'resolve_annotation') {
    ...
    $result = resolve_final_routing_annotation($conn, $annotation_id);
    if (!$result['success']) {
        http_response_code(400);
        echo json_encode($result);
        exit;
...
if ($action === 'delete_annotation') {
    ...
    $result = delete_final_routing_annotation($conn, $annotation_id);
    if (!$result['success']) {
        http_response_code(400);
        echo json_encode($result);
        exit;
...
if ($action === 'add_reply') {
    ...
    $result = add_final_routing_annotation_reply($conn, $annotation_id, $user_id, $reply_content, $reply_user_role);
    if (!$result['success']) {
        http_response_code(400);
        echo json_encode($result);
        exit;
    ...
    $submission = fetch_final_routing_submission($conn, (int)$annotation['submission_id']);
    if ($submission) {
        if ($reply_user_role === 'student') {
            ...
            if ($stmt) {
                ...
                while ($row = $res->fetch_assoc()) {
                    ...
                    notify_user(
                        $conn,
                        $rid,
                        'Student reply to final routing annotation',
                        'A student replied to a final routing annotation.',
                        "committee_final_routing_review.php?submission_id={$annotation['submission_id']}",
                        true
                    );
                }
                $stmt->close();
            ...
            notify_user(
                ...
                "student_final_routing_view.php?submission_id={$annotation['submission_id']}"
            );
        }
    }
...
if ($action === 'fetch_statistics') {
    ...
    $stats = get_final_routing_annotation_statistics($conn, $submission_id);
    http_response_code(200);
