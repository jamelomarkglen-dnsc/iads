...
$logStmt = $conn->prepare(
    "SELECT l.id, l.old_status, l.new_status, l.changed_at,
            s.title, s.type,
            u.firstname, u.lastname, u.email, u.role AS updater_role
     FROM status_logs l
...
<head>
  <meta charset="UTF-8">
  <title>Dean Activity Log - DNSC IAdS</title>
...
<body>
    ...
    <div class="container my-4">
      <div class="page-header">
        <div class="page-title">
          ...
          <div>
            <h3 class="fw-bold mb-1">Dean Activity Log</h3>
            <p class="text-muted mb-0">System-wide status updates for quick oversight.</p>
          </div>
        </div>
          ...
          <div class="card summary-card h-100">
            <div class="card-body">
              ...
              <div class="stat-value" style="font-size: 1.2rem;">
                <?= $lastUpdated ? date('M d, Y g:i A', strtotime($lastUpdated)) : 'No updates yet'; ?>
              </div>
              <div class="text-muted small mt-2">Showing most recent <?= $limit; ?> updates.</div>
            </div>
          </div>
        </div>
      </div>
            ...
            <div class="col-lg-3 col-md-6">
              ...
              <select id="typeFilter" class="form-select">
                ...
                <?php foreach ($types as $typeOption): ?>
                  <?php if ($typeOption === '') continue; ?>
                  <option value="<?= htmlspecialchars($typeOption); ?>"><?= htmlspecialchars($typeOption); ?></option>
                <?php endforeach; ?>
              </select>
            </div>
            <div class="col-lg-3 col-md-6">
              <label for="updaterFilter" class="form-label">Updated By</label>
              <input type="search" id="updaterFilter" class="form-control" placeholder="Name or role...">
            </div>
            <div class="col-lg-3 col-md-6">
              <label for="searchInput" class="form-label">Search</label>
              <input type="search" id="searchInput" class="form-control" placeholder="Search by title or status...">
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="card timeline-card">
        <div class="timeline-header">
          ...
          <span class="badge bg-light text-dark px-3 py-2" id="timelineCount"><?= number_format($totalLogs); ?> entries</span>
        </div>
        <ul class="timeline-list" id="timelineList">
          <?php if (count($logs) === 0): ?>
            <li class="empty-state">
              <i class="bi bi-inboxes"></i>
              <p class="mb-0">No status changes logged yet.</p>
            </li>
          <?php else: foreach ($logs as $log):
              ...
              $updaterRole = $log['updater_role'] ?? '';
              $updaterRoleLabel = $updaterRole ? getRoleLabel($updaterRole) : '';
              $dataSearch = strtolower($title . ' ' . $type . ' ' . $updatedBy . ' ' . $updaterRole . ' ' . $updaterRoleLabel . ' ' . $newStatus . ' ' . $oldStatus);
          ?>
            <li class="timeline-item"
                data-status="<?= htmlspecialchars($newStatus); ?>"
                data-type="<?= htmlspecialchars($type); ?>"
                data-updater="<?= htmlspecialchars(strtolower($updatedBy . ' ' . $updaterRole . ' ' . $updaterRoleLabel)); ?>"
              ...
              <div class="log-card">
                ...
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
                  ...
                  <span class="user-badge">
                    ...
                    <?php if (!empty($updaterRoleLabel)): ?>
                      <span class="text-muted">· <?= htmlspecialchars($updaterRoleLabel); ?></span>
                    <?php endif; ?>
                  </span>
                </div>
              </div>
  ...
  <script>
    (function () {
      ...
      const updaterFilter = document.getElementById('updaterFilter');
      ...
      const handleFilters = () => {
        ...
        const updaterValue = (updaterFilter?.value || '').toLowerCase();
        ...
        items.forEach((item) => {
          ...
          const matchesUpdater = !updaterValue || (item.dataset.updater || '').includes(updaterValue);
          const matchesSearch = !searchValue || (item.dataset.search || '').includes(searchValue);
          const shouldShow = matchesStatus && matchesType && matchesUpdater && matchesSearch;
        ...
        if (countLabel) {
          countLabel.textContent = visibleCount + ' entr' + (visibleCount === 1 ? 'y' : 'ies');
        }
      };

      statusFilter?.addEventListener('change', handleFilters);
      typeFilter?.addEventListener('change', handleFilters);
      updaterFilter?.addEventListener('input', handleFilters);
