...
$columnCheck = $conn->query("SHOW COLUMNS FROM defense_panels LIKE 'panel_member_id'");
if ($columnCheck) {
...
$roleColumnCheck = $conn->query("SHOW COLUMNS FROM defense_panels LIKE 'panel_role'");
if ($roleColumnCheck) {
    if ($roleColumnCheck->num_rows === 0) {
...
$panelResponseFilter = '';
$responseColumnExistsResult = $conn->query("SHOW COLUMNS FROM defense_panels LIKE 'response'");
if ($responseColumnExistsResult && $responseColumnExistsResult->num_rows > 0) {
    $panelResponseFilter = "AND (dp.response IS NULL OR dp.response = '' OR dp.response = 'Accepted')";
}
if ($responseColumnExistsResult) {
    $responseColumnExistsResult->free();
}
...
$sql = "
  ...
  WHERE (dp.panel_member_id = ? OR (dp.panel_member_id IS NULL AND dp.panel_member LIKE CONCAT('%', ?, '%'))) {$panelResponseFilter}
  GROUP BY ds.id
  ORDER BY ds.defense_date ASC
";
$stmt = $conn->prepare($sql);
$stmt->bind_param("is", $panelUserId, $panelName);
$stmt->execute();
  ...
  <div class="container-fluid">
    ...
    <div class="card shadow-sm">
      ...
      <div class="card-body">
        ...
        <?php else: ?>
          <?php $panelMembersStmt = $conn->prepare("SELECT panel_member, panel_role FROM defense_panels WHERE defense_id = ? ORDER BY id"); ?>
          <?php foreach ($schedule as $assignment): ?>
            <?php
              ...
              $panelSummary = '';
              if ($panelMembersStmt) {
                  $panelMembersStmt->bind_param('i', $assignment['defense_id']);
                  ...
                  foreach ($panelRows as $row) {
                      $name = trim($row['panel_member'] ?? '');
                      if ($name === '') {
                          continue;
                      }
                      $label = $roleLabels[$row['panel_role'] ?? 'panel_member'] ?? 'Panel';
                      $panelSegments[] = "{$label}: {$name}";
                  }
                  $panelSummary = implode(', ', $panelSegments);
              }
              $searchable = strtolower(($assignment['student_name'] ?? '') . ' ' . ($assignment['submission_title'] ?? '') . ' ' . $status . ' ' . $panelSummary);
            ?>
            <div class="schedule-card bg-white p-4 mb-4" data-search="<?= htmlspecialchars($searchable); ?>">
              ...
              <?php if (!empty($panelSummary)): ?>
                <div class="text-muted mb-0 small">
                  <strong><i class="bi bi-people-fill me-1"></i>Panel:</strong>
                  <?= htmlspecialchars($panelSummary); ?>
                </div>
              <?php endif; ?>
            </div>
          <?php endforeach; ?>
