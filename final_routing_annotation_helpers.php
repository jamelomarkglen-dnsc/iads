<?php
/**
 * Final Routing Annotation Helpers
 */

require_once 'db.php';

function create_final_routing_annotation(
    mysqli $conn,
    int $submission_id,
    int $reviewer_id,
    ...
    $stmt = $conn->prepare("
        INSERT INTO final_routing_annotations (
            submission_id,
            reviewer_id,
            reviewer_role,
            annotation_type,
...
function fetch_final_routing_annotation(mysqli $conn, int $annotation_id): ?array
{
    $stmt = $conn->prepare("
        SELECT *
        FROM final_routing_annotations
        WHERE annotation_id = ?
        LIMIT 1
    ");
    if (!$stmt) {
...
function fetch_final_routing_submission_annotations(mysqli $conn, int $submission_id): array
{
    $stmt = $conn->prepare("
        SELECT
            a.*,
        ...
        FROM final_routing_annotations a
...
function fetch_final_routing_page_annotations(mysqli $conn, int $submission_id, int $page_number): array
{
    $stmt = $conn->prepare("
        SELECT
            a.*,
...
function update_final_routing_annotation(mysqli $conn, int $annotation_id, string $annotation_content, int $changed_by): array
{
    $annotation_content = trim($annotation_content);
    if ($annotation_content === '') {
    ...
    $stmt = $conn->prepare("
        UPDATE final_routing_annotations
...
function resolve_final_routing_annotation(mysqli $conn, int $annotation_id): array
{
    $stmt = $conn->prepare("
        UPDATE final_routing_annotations
...
function delete_final_routing_annotation(mysqli $conn, int $annotation_id): array
{
    $stmt = $conn->prepare("
        DELETE FROM final_routing_annotations
        WHERE annotation_id = ?
    ");
    if (!$stmt) {
        return ['success' => false, 'error' => 'Database error: ' . $conn->error];
...
function add_final_routing_annotation_reply(mysqli $conn, int $annotation_id, int $user_id, string $reply_content, string $reply_user_role): array
{
    ...
    $stmt = $conn->prepare("
        INSERT INTO final_routing_annotation_replies (
            annotation_id,
            user_id,
            reply_content,
            reply_user_role
...
function fetch_final_routing_annotation_replies(mysqli $conn, int $annotation_id): array
{
    $stmt = $conn->prepare("
        SELECT
            r.reply_id,
        ...
        FROM final_routing_annotation_replies r
...
function get_final_routing_annotation_statistics(mysqli $conn, int $submission_id): array
{
    $stmt = $conn->prepare("
        SELECT
