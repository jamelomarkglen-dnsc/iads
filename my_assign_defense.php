...
function formatDefenseScheduleLabel(?string $date, ?string $time): string
{
    ...
    if ($date !== '' && ($parsedDate = strtotime($date))) {
        return date('F d, Y', $parsedDate);
    }
    if ($time !== '' && ($parsedTime = strtotime($time))) {
...
$panelName = trim($panelName) !== '' ? trim($panelName) : 'Panel Member';

$search = trim($_GET['search'] ?? '');
$statusFilter = trim($_GET['status'] ?? '');

$conditions = [
    "dp.panel_role = 'panel_member'",
...
$panelMembersStmt = $conn->prepare("
    SELECT panel_member, panel_role
    FROM defense_panels
    WHERE defense_id = ?
    ORDER BY id
");
...
<head>
    <meta charset="UTF-8">
    <title>My Assigned Defenses - DNSC IAdS</title>
    ...
    <style>
        ...
        .panel-table thead th { font-size: 0.85rem; text-transform: uppercase; letter-spacing: 0.05em; color: #556; }
...
<div class="content">
    <div class="container-fluid">
        <div class="d-flex justify-content-between align-items-start flex-wrap mb-4">
            <div>
                <h3 class="fw-bold text-success mb-1"><i class="bi bi-calendar-check me-2"></i>My Assigned Defenses</h3>
                <p class="text-muted mb-0">Schedules assigned to you as part of the defense panel.</p>
            </div>
        ...
        <?php if (empty($assignments)): ?>
            <div class="alert alert-light border text-center">
                ...
                No defense schedules assigned to you yet.
            </div>
        <?php else: ?>
            <div class="card shadow-sm border-0">
                <div class="card-body p-0">
                    <div class="table-responsive">
                        <table class="table table-hover align-middle mb-0 panel-table">
                                            ...
                                            <?php else: ?>
                                                <?php foreach ($panelRows as $memberRow): ?>
                                                    <?php
                                                        ...
                                                        $role = $roleLabels[$memberRow['panel_role'] ?? 'panel_member'] ?? 'Panel Member';
                                                    ?>
                                                    <span class="panel-chip">
                                                        <i class="bi bi-person"></i>
                                                        <strong><?= htmlspecialchars($role); ?>:</strong> <?= htmlspecialchars($memberName); ?>
                                                    </span>
                                                <?php endforeach; ?>
                                            <?php endif; ?>
                                        </td>
