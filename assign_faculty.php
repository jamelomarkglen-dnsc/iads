...
$reviewerRoleLabels = [
    'faculty' => 'Faculty Reviewers',
];
$reviewerDashboardLinks = [
    'faculty' => 'subject_specialist_dashboard.php',
];
...
{
    $sql = "
        SELECT cp.id, cp.title, cp.assigned_faculty, cp.student_id, cp.created_at,
               u.firstname, u.lastname, u.email
        FROM concept_papers cp
        LEFT JOIN users u ON u.id = cp.student_id
        ORDER BY u.lastname ASC, cp.created_at ASC
    ";
    $result = $conn->query($sql);
    $groups = [];
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            ...
            if (!isset($groups[$studentId])) {
                $groups[$studentId] = [
                    ...
                    'student_name' => trim(($row['firstname'] ?? '') . ' ' . ($row['lastname'] ?? '')) ?: 'Unknown Student',
                    'student_email' => $row['email'] ?? 'Not provided',
                    'papers' => [],
                ];
            }
            $groups[$studentId]['papers'][] = [
                'id' => (int)$row['id'],
...
/**
 * Sync reviewer assignments per role and concept paper.
 *
 * @param array<int,int> $paperIds
 * @param array<string,array<int>> $roleAssignments
 * @return array<string,array<int>> Newly added reviewer IDs per role
 */
function syncReviewerAssignments(mysqli $conn, array $paperIds, int $studentId, array $roleAssignments, ?string $instructions, ?string $dueDate, int $assignerId): array
{
    ensureConceptReviewTables($conn);
    ...
    $insertStmt = $conn->prepare("
        INSERT INTO concept_reviewer_assignments (concept_paper_id, student_id, reviewer_id, reviewer_role, assigned_by, instructions, due_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
        ON DUPLICATE KEY UPDATE instructions = VALUES(instructions), due_at = VALUES(due_at), assigned_by = VALUES(assigned_by), status = 'pending'
    ");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['assign_reviewers'])) {
    $facultySelections = array_values(array_filter([
        (int)($_POST['faculty_id_1'] ?? 0),
        (int)($_POST['faculty_id_2'] ?? 0),
        (int)($_POST['faculty_id_3'] ?? 0),
    ]));
    ...
    if (!$message) {
        ...
        $newAssignments = syncReviewerAssignments(
            $conn,
            $paperIds,
            $studentId,
            $roleAssignments,
        ...
        if (!empty($reviewersToNotify)) {
            foreach ($newAssignments as $roleKey => $ids) {
                ...
                $body = "You have been assigned as {$roleLabel} to review the concept titles submitted by {$studentName}. Please rate each title and recommend which one to pursue.";
                if ($dueDate) {
...
foreach ($conceptGroups as &$group) {
    ...
    foreach ($group['papers'] as &$paper) {
        ...
        $paperAssignments = $assignmentIndex[$paperId] ?? [
            'all' => [],
            'by_role' => [],
...
$pipelineStages = [
    'concept' => ['label' => 'Concept Papers', 'icon' => 'bi-layers-half', 'color' => 'success', 'total' => 0, 'pending' => 0, 'in_review' => 0, 'approved' => 0],
    'thesis' => ['label' => 'Thesis Manuscripts', 'icon' => 'bi-journal-text', 'color' => 'primary', 'total' => 0, 'pending' => 0, 'in_review' => 0, 'approved' => 0],
    'capstone' => ['label' => 'Capstone Projects', 'icon' => 'bi-layers', 'color' => 'info', 'total' => 0, 'pending' => 0, 'in_review' => 0, 'approved' => 0],
    'dissertation' => ['label' => 'Dissertation Tracks', 'icon' => 'bi-mortarboard', 'color' => 'warning', 'total' => 0, 'pending' => 0, 'in_review' => 0, 'approved' => 0],
    'others' => ['label' => 'Other Research', 'icon' => 'bi-columns-gap', 'color' => 'secondary', 'total' => 0, 'pending' => 0, 'in_review' => 0, 'approved' => 0],
];
...
if ($submissionsTableExists) {
    ...
    $pipelineSql = "
        SELECT {$typeExpr} AS type_key,
               {$statusExpr} AS status_key,
               COUNT(*) AS total
        FROM submissions
        GROUP BY type_key, status_key
    ";
...
if ($submissionsTableExists) {
    ...
    $studentNameSelect = $submissionsHasStudent
    ...
    $submissionSql = "
        SELECT s.id,
               s.title,
               {$typeSelect} AS submission_type,
               {$statusSelect} AS submission_status,
               {$createdSelect} AS submission_created_at,
               {$studentNameSelect} AS student_name,
    ...
    if ($submissionResult = $conn->query($submissionSql)) {
        while ($row = $submissionResult->fetch_assoc()) {
            $latestSubmissionFeed[] = [
                ...
                'type' => $row['submission_type'] ?? 'Concept Paper',
                'status' => $row['submission_status'] ?? 'Pending',
                'created_at' => $row['submission_created_at'] ?? null,
...
<head>
    <meta charset="UTF-8">
    <title>Assign Faculty Reviewers</title>
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.2/dist/css/bootstrap.min.css">
    <style>
        body { background: #f6f8fb; }
        .content { margin-left: 220px; padding: 24px; min-height: 100vh; transition: margin-left .3s; }
        #sidebar.collapsed ~ .content { margin-left: 60px; }
        .hero-card { border-radius: 22px; background: linear-gradient(135deg, #16562c, #0f3d1f); color: #fff; padding: 28px; box-shadow: 0 22px 48px rgba(15, 61, 31, 0.25); }
        .pipeline-card { border-radius: 24px; box-shadow: 0 24px 60px rgba(15,61,31,.06); border: none; background: #fff; }
        .stage-item { border-radius: 18px; border: 1px solid rgba(22,86,44,.08); padding: 1rem; background: #fefefe; height: 100%; }
        .stage-pill { width: 46px; height: 46px; border-radius: 14px; background: rgba(22,86,44,.12); display: inline-flex; align-items: center; justify-content: center; font-size: 1.25rem; }
        .stage-progress { height: 6px; border-radius: 999px; background: rgba(12,53,22,.1); overflow: hidden; }
        .intake-card { border-radius: 22px; border: none; box-shadow: 0 20px 40px rgba(15,61,31,.08); }
        .intake-entry { padding: 0.85rem 0; border-bottom: 1px solid rgba(15,61,31,.08); display: flex; justify-content: space-between; gap: 1rem; flex-wrap: wrap; }
        .intake-entry:last-child { border-bottom: none; }
        .stat-card { border-radius: 18px; box-shadow: 0 18px 36px rgba(15,61,31,.08); border: none; }
        .student-card { border-radius: 22px; box-shadow: 0 18px 40px rgba(15,61,31,.12); border: none; overflow: hidden; }
        .student-card .list-group-item { border: none; border-bottom: 1px solid rgba(15,61,31,.08); }
        .keyword-badge { border-radius: 999px; background: rgba(15,61,31,.08); padding: .2rem .6rem; font-size: .85rem; margin-right: .25rem; }
        .assign-form select { min-height: 48px; }
        .assign-form textarea { resize: vertical; }
        .review-brief { border-radius: 16px; border: 1px dashed rgba(22,86,44,.25); background: #f4f8f2; padding: 0.85rem 1rem; }
        .review-brief .badge { margin-right: .5rem; }
        .status-pill {
            border-radius: 999px;
        ...
        .role-icon {
            ...
            height: 38px;
            border-radius: 12px;
            background: rgba(22,86,44,.12);
            display: inline-flex;
            align-items: center;
            justify-content: center;
            color: #16562c;
        }
        .role-description { font-size: 0.8rem; }
        .reviewer-pill {
            border-radius: 999px;
        ...
        .reviewer-pill.completed { background: rgba(72,180,97,0.15); border-color: rgba(72,180,97,0.4); color: #0f6631; }
        .reviewer-pill.pending { background: rgba(255,193,7,0.15); border-color: rgba(255,193,7,0.4); color: #945400; }
        .reviewer-pool-card { border-radius: 18px; border: none; box-shadow: 0 18px 36px rgba(15,61,31,.08); height: 100%; }
        ...
        .reviewer-list li:last-child { border-bottom: 0; }
        .reviewer-chip { background: rgba(22,86,44,.08); border-radius: 999px; padding: 0.15rem 0.65rem; font-size: 0.85rem; color: #16562c; }
        .assignment-layout {
            display: grid;
        ...
        .assign-panel {
            border-radius: 22px;
            border: 1px solid rgba(22,86,44,.12);
            background: #fff;
            padding: 1.25rem;
            box-shadow: 0 18px 36px rgba(15,61,31,.08);
            height: 100%;
        }
        .concept-panel {
            display: flex;
        ...
        .quick-assign-box {
            border: 1px dashed rgba(22,86,44,.25);
            border-radius: 18px;
            background: #f8fbf7;
        ...
        .quick-reviewer-card {
            border: 1px solid rgba(22,86,44,.18);
            border-radius: 16px;
            padding: 0.75rem;
            background: #fff;
            text-align: center;
            box-shadow: 0 12px 24px rgba(15,61,31,.08);
            display: flex;
            flex-direction: column;
            gap: 0.5rem;
        }
        .quick-reviewer-card .form-label {
            font-weight: 700;
            margin-bottom: 0;
        }
        .quick-reviewer-card select {
            border-radius: 12px;
            text-align: center;
            font-weight: 600;
        }
        ...
        .reviewer-role-group {
            ...
            padding: 0.85rem 1rem;
            background: #fff;
            box-shadow: 0 12px 24px rgba(15,61,31,0.05);
            display: flex;
            flex-direction: column;
            gap: 0.5rem;
        }
        .reviewer-role-group .role-title {
            display: block;
            font-size: 0.95rem;
            font-weight: 700;
            text-transform: uppercase;
        ...
        .reviewer-role-group select {
            border-radius: 14px;
            border: 1px solid rgba(22,86,44,0.2);
        }
        .reviewer-role-group select:focus {
            border-color: #16562c;
            box-shadow: 0 0 0 0.15rem rgba(22,86,44,.15);
        }
        .reviewer-role-group small { font-size: 0.75rem; }
        .instructions-box {
            border-radius: 18px;
            border: 1px solid rgba(22,86,44,.12);
            background: #fff;
    ...
    <div class="container-fluid">
        <div class="hero-card mb-4">
            <div class="d-flex flex-column flex-lg-row justify-content-between gap-3">
                <div>
                    <p class="text-uppercase small mb-1">Program Chairperson Workspace</p>
                    <h2 class="fw-bold mb-2">Assign Faculty Reviewers</h2>
                    <p class="mb-0">Coordinate concept papers, theses, capstones, and dissertations. Route each student's three concept titles to expert reviewers so they can mark Rank 1, 2, or 3 and endorse the final topic.</p>
                </div>
                <div class="text-end">
                    <span class="badge bg-light text-success fs-6"><?= number_format($totalPapers); ?> concept titles</span>
                    <div class="small text-white-50 mt-1">Reviewer tasks in progress: <?= number_format($assignmentStats['pending'] ?? 0); ?></div>
                </div>
            </div>
        </div>

        <?php if (!empty($pipelineStageDisplay)): ?>
        <div class="card pipeline-card mb-4">
            <div class="card-body">
                <div class="d-flex flex-column flex-lg-row justify-content-between align-items-start align-items-lg-center gap-3 mb-3">
                    <div>
                        <p class="text-uppercase small text-muted mb-1">Research Lifecycle</p>
                        <h3 class="fw-semibold mb-1">Monitor Intake to Evaluation</h3>
                        <p class="text-muted mb-0"><?= number_format($pipelineTotalCount ?: $totalPapers); ?> total submissions across concept papers, theses, capstones, and dissertations.</p>
                    </div>
                    <a href="submissions.php" class="btn btn-outline-success">
                        <i class="bi bi-folder-check me-1"></i> Manage Submissions
                    </a>
                </div>
                <div class="row g-3">
                    <?php foreach ($pipelineStageDisplay as $stageKey => $stageData): ?>
                        <?php
                            $stageTotal = max(0, (int)($stageData['total'] ?? 0));
                            $stagePending = max(0, (int)($stageData['pending'] ?? 0));
                            $stageReview = max(0, (int)($stageData['in_review'] ?? 0));
                            $stageApproved = max(0, (int)($stageData['approved'] ?? 0));
                            $completionPercent = $stageTotal > 0 ? round(($stageApproved / $stageTotal) * 100) : 0;
                            $theme = $stageData['color'] ?? 'success';
                            $icon = $stageData['icon'] ?? 'bi-diagram-3';
                        ?>
                        <div class="col-12 col-sm-6 col-xl-3">
                            <div class="stage-item h-100">
                                <div class="d-flex align-items-center justify-content-between mb-3">
                                    <div class="stage-pill text-<?= htmlspecialchars($theme); ?>">
                                        <i class="bi <?= htmlspecialchars($icon); ?>"></i>
                                    </div>
                                    <span class="badge bg-light text-<?= htmlspecialchars($theme); ?>"><?= number_format($stageTotal); ?> files</span>
                                </div>
                                <p class="fw-semibold mb-1 text-<?= htmlspecialchars($theme); ?>"><?= htmlspecialchars($stageData['label']); ?></p>
                                <div class="progress stage-progress mb-2">
                                    <div class="progress-bar bg-<?= htmlspecialchars($theme); ?>" role="progressbar" style="width: <?= $completionPercent; ?>%;" aria-valuenow="<?= $completionPercent; ?>" aria-valuemin="0" aria-valuemax="100"></div>
                                </div>
                                <div class="d-flex justify-content-between text-muted small">
                                    <span><?= number_format($stagePending); ?> pending</span>
                                    <span><?= number_format($stageReview); ?> in-review</span>
                                    <span><?= number_format($stageApproved); ?> cleared</span>
                                </div>
                            </div>
                        </div>
                    <?php endforeach; ?>
        ...
        <?php if (!empty($conceptGroups)): ?>
            <div class="row g-4" id="studentCards">
                <?php foreach ($visibleConceptGroups as $group): ?>
                    <?php
                        $needsReviewers = !empty($group['needs_reviewers']);
                        $firstPaper = $group['papers'][0] ?? null;
                        $roleSelections = [];
                        foreach ($reviewerRoles as $roleKey) {
                            $roleSelections[$roleKey] = [];
                            if ($firstPaper && !empty($firstPaper['review_assignments']['by_role'][$roleKey])) {
                                $roleSelections[$roleKey] = array_values(array_unique(array_map(
                                    fn($assignment) => (int)($assignment['reviewer_id'] ?? 0),
                                    $firstPaper['review_assignments']['by_role'][$roleKey]
                                )));
                            }
                            if ($roleKey === 'faculty' && empty($roleSelections[$roleKey]) && $firstPaper) {
                                $legacy = array_filter(array_map('intval', explode(',', (string)($firstPaper['assigned_faculty'] ?? ''))));
                                if (!empty($legacy)) {
                                    $roleSelections[$roleKey] = $legacy;
                                }
                            }
                        }
                        $facultySelections = array_values(array_unique($roleSelections['faculty'] ?? []));
                        $facultySelections = array_pad(array_slice($facultySelections, 0, 3), 3, 0);
                        $dueAtValue = '';
                        if (!empty($group['due_at'])) {
                            $dueTimestamp = strtotime($group['due_at']);
                            if ($dueTimestamp) {
                                $dueAtValue = date('Y-m-d', $dueTimestamp);
                            }
                        }
                    ?>
                        ...
                        <div class="card student-card h-100">
                            <div class="card-body">
                                <div class="d-flex justify-content-between align-items-start mb-3">
                                    <div>
                                        <h5 class="text-success mb-1"><?= htmlspecialchars($group['student_name']); ?></h5>
                                        <small class="text-muted"><?= htmlspecialchars($group['student_email']); ?></small>
                                    </div>
                                    <span class="badge <?= $needsReviewers ? 'bg-warning-subtle text-warning' : 'bg-success-subtle text-success'; ?>">
                                        <?= $needsReviewers ? 'Needs reviewers' : 'Assigned'; ?>
                                    </span>
                                </div>

                                <form method="POST" class="assign-form">
                                    <input type="hidden" name="assign_reviewers" value="1">
                                    <input type="hidden" name="student_id" value="<?= (int)$group['student_id']; ?>">
                                    <?php foreach ($group['papers'] as $paper): ?>
                                        <input type="hidden" name="paper_ids[]" value="<?= (int)$paper['id']; ?>">
                                    <?php endforeach; ?>

                                    <div class="assignment-layout">
                                        <div class="concept-panel">
                                            <div class="concept-header">
                                                <span class="concept-chip">Name of student</span>
                                                <h5 class="mb-0"><?= htmlspecialchars($group['student_name']); ?></h5>
                                                <small class="text-muted"><?= htmlspecialchars($group['student_email']); ?></small>
                                            </div>
                                            <div>
                                                <p class="fw-semibold mb-2">Concept titles</p>
                                                <div class="concept-stack">
                                                    <?php foreach ($group['papers'] as $paper): ?>
                                                        <?php
                                                            $isPending = empty($paper['review_assignments']['all']) && trim((string)$paper['assigned_faculty']) === '';
                                                            $statusClass = $isPending ? 'bg-secondary-subtle text-secondary' : 'bg-success-subtle text-success';
                                                            $statusLabel = $isPending ? 'Pending' : 'In review';
                                                        ?>
                                                        <div class="concept-row">
                                                            <div>
                                                                <h6><?= htmlspecialchars($paper['title']); ?></h6>
                                                                <div class="concept-meta">Submitted <?= htmlspecialchars(formatDateToReadable($paper['created_at'] ?? '')); ?></div></div>
                                            ...
                                            <div class="instructions-box concept-instructions-box">
                                                <div>
                                                    <p class="text-uppercase small text-muted fw-semibold mb-1">Review instruction portion</p>
                                                    <textarea class="form-control" name="instructions" rows="2" placeholder="Explain how reviewers should rate the student's titles"><?= htmlspecialchars($group['instructions']); ?></textarea>
                                                </div>
                                                <div>
                                                    <p class="text-uppercase small text-muted fw-semibold mb-1">Date for calendar</p>
                                                    <div class="input-group">
                                                        <span class="input-group-text bg-white"><i class="bi bi-calendar-event text-success"></i></span>
                                                        <input type="date" class="form-control" name="due_at" value="<?= htmlspecialchars($dueAtValue); ?>">
                                                    </div>
                                                </div>
                                            </div>

                                            <?php
                                                $reviewerSummaryNames = [];
                                                $reviewerSummaryIndex = [];
                                                if ($firstPaper && !empty($firstPaper['review_assignments']['all'])) {
                                                    foreach ($firstPaper['review_assignments']['all'] as $assignment) {
                                                        $rid = (int)($assignment['reviewer_id'] ?? 0);
                                                        if ($rid <= 0 || isset($reviewerSummaryIndex[$rid])) {
                                                            continue;
                                                        }
                                                        $info = $reviewerLookup[$rid] ?? null;
                                                        if ($info) {
                                                            $name = trim(($info['firstname'] ?? '') . ' ' . ($info['lastname'] ?? ''));
                                                            if ($name !== '') {
                                                                $reviewerSummaryIndex[$rid] = true;
                                                                $reviewerSummaryNames[] = $name;
                                                            }
                                                        }
                                                    }
                                                }
                                                if (empty($reviewerSummaryNames) && $firstPaper) {
                                                    $legacyIds = array_filter(array_map('intval', explode(',', (string)($firstPaper['assigned_faculty'] ?? ''))));
                                                    foreach ($legacyIds as $rid) {
                                                        if ($rid <= 0 || isset($reviewerSummaryIndex[$rid])) {
                                                            continue;
                                                        }
                                                        $info = $reviewerLookup[$rid] ?? null;
                                            ...
                                            ?>
                                            <?php if (!empty($reviewerSummaryNames)): ?>
                                                <div class="role-summary-grid">
                                                    <div class="role-summary-card">
                                                        <p>Faculty reviewers</p>
                                                        <?php foreach ($reviewerSummaryNames as $name): ?>
                                                            <span class="keyword-badge"><?= htmlspecialchars($name); ?></span>
                                                        <?php endforeach; ?>
                                                    </div>
                                                </div>
                                            <?php endif; ?>
                                        ...
                                        <div class="assign-panel">
                                            <div class="reviewer-role-strip">
                                                <?php foreach ($reviewerRoles as $roleKey): ?>
                                                <div class="reviewer-role-card">
                                                        <div class="reviewer-role-group">
                                                            <span class="role-title"><?= htmlspecialchars($reviewerRoleLabels[$roleKey] ?? 'Faculty Reviewers'); ?></span>
                                                            <div class="row g-2">
                                                                <?php foreach ([1, 2, 3] as $slot): ?>
                                                                    <?php $selectedId = (int)($facultySelections[$slot - 1] ?? 0); ?>
                                                                    <div class="col-12 col-md-4">
                                                                        <select class="form-select form-select-sm reviewer-select faculty-select" data-role="<?= htmlspecialchars($roleKey); ?>" data-slot="<?= $slot; ?>" name="faculty_id_<?= $slot; ?>">
                                                                            <option value="" <?= $selectedId === 0 ? 'selected' : ''; ?>>Select Reviewer</option>
                                                                            <?php foreach (($reviewerDirectory['by_role'][$roleKey] ?? []) as $reviewerId => $reviewer): ?>
                                                                                <option value="<?= $reviewerId; ?>" <?= $reviewerId === $selectedId ? 'selected' : ''; ?>>
                                                                                    <?= htmlspecialchars(trim(($reviewer['firstname'] ?? '') . ' ' . ($reviewer['lastname'] ?? ''))); ?>
                                                                                </option>
                                                                            <?php endforeach; ?>
                                                                        </select>
                                                                    </div>
                                                            ...
                                                            <small class="text-muted d-block mt-1">Choose up to three distinct faculty reviewers.</small>
                                                        </div>
                                                    </div>
                                                <?php endforeach; ?>
                                            </div>
                                            ...
                                            <div class="d-flex justify-content-between align-items-center">
                                                <small class="text-muted">Reviewers will rate all titles and recommend one to pursue.</small>
                                                <button type="submit" class="btn btn-success">Save Assignment</button>
                                            </div>

                                            <?php $studentSummary = $assignedSummaryIndex[$group['student_id']] ?? null; ?>
                                            <div class="card assigned-summary-card">
                                                <div class="card-body">
                                                    <p class="text-uppercase small text-muted fw-semibold mb-1">Assigned Evaluators</p>
                                                    <h5 class="fw-semibold mb-3">Current faculty reviewers</h5>
                                                    <div class="table-responsive">
                                                        <table class="table align-middle mb-0">
                                                            <thead class="table-light">
                                                                <tr>
                                                                    <th>Student</th>
                                                                    <th>Faculty Reviewers</th>
                                                                </tr>
                                                            </thead>
                                                            <tbody>
                                                                <?php if (empty($studentSummary)): ?>
                                                                    <tr>
                                                                        <td colspan="2" class="text-muted">No evaluators assigned yet. Assign faculty reviewers to see them listed here.</td>
                                                                    </tr>
                                                                <?php else: ?>
                                                                    <tr>
                                                                        <td class="fw-semibold text-success"><?= htmlspecialchars($group['student_name']); ?></td>
                                                                        <?php foreach (['Faculty'] as $roleLabel): ?>
                                                                            <?php $names = $studentSummary['roles'][$roleLabel] ?? []; ?>
                                                                            <td>
                                                                                <?php if (empty($names)): ?>
                                                                                    <span class="text-muted small">None</span>
                                                                                <?php else: ?>
                                                                                    <?php foreach ($names as $name): ?>
                                                                                        <span class="badge bg-light text-success border me-1 mb-1"><?= htmlspecialchars($name); ?></span>
                                                                                    <?php endforeach; ?>
                                                                                <?php endif; ?>
                                                                            </td>
                                                                        <?php endforeach; ?>
        ...
        const updateOptions = () => {
            ...
            selects.forEach((select) => {
                ...
                Array.from(select.options).forEach((option) => {
                    ...
                    option.hidden = selectedSet.has(option.value) && option.value !== currentValue;
                });
            });
        };
