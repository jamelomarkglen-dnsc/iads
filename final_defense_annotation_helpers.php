<?php
/**
 * Final Defense Annotation Helpers
 */

require_once 'db.php';

function ensureFinalDefenseAnnotationTables(mysqli $conn): void
{
    ...
    $conn->query("
        CREATE TABLE IF NOT EXISTS final_defense_annotations (
            ...
            INDEX idx_final_defense_annotation_submission (submission_id),
            INDEX idx_final_defense_annotation_reviewer (reviewer_id)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci
    ");

    $conn->query("
        CREATE TABLE IF NOT EXISTS final_defense_annotation_replies (
            ...
            INDEX idx_final_defense_reply_annotation (annotation_id)
        ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci
    ");
...
function create_final_defense_annotation(
    mysqli $conn,
    int $submission_id,
    int $reviewer_id,
    ...
    $stmt = $conn->prepare("
        INSERT INTO final_defense_annotations (
            submission_id,
            reviewer_id,
            reviewer_role,
            annotation_type,
...
function fetch_final_defense_annotation(mysqli $conn, int $annotation_id): ?array
{
    $stmt = $conn->prepare("
        SELECT *
        FROM final_defense_annotations
        WHERE annotation_id = ?
        LIMIT 1
    ");
    if (!$stmt) {
...
function fetch_final_defense_submission_annotations(mysqli $conn, int $submission_id): array
{
    $stmt = $conn->prepare("
        SELECT
            a.*,
        ...
        FROM final_defense_annotations a
...
function fetch_final_defense_page_annotations(mysqli $conn, int $submission_id, int $page_number): array
{
    $stmt = $conn->prepare("
        SELECT
            a.*,
...
function update_final_defense_annotation(mysqli $conn, int $annotation_id, string $annotation_content, int $changed_by): array
{
    $annotation_content = trim($annotation_content);
    if ($annotation_content === '') {
    ...
    $stmt = $conn->prepare("
        UPDATE final_defense_annotations
...
function resolve_final_defense_annotation(mysqli $conn, int $annotation_id): array
{
    $stmt = $conn->prepare("
        UPDATE final_defense_annotations
...
function delete_final_defense_annotation(mysqli $conn, int $annotation_id): array
{
    $stmt = $conn->prepare("
        DELETE FROM final_defense_annotations
        WHERE annotation_id = ?
    ");
    if (!$stmt) {
        return ['success' => false, 'error' => 'Database error: ' . $conn->error];
...
function add_final_defense_annotation_reply(mysqli $conn, int $annotation_id, int $user_id, string $reply_content, string $reply_user_role): array
{
    ...
    $stmt = $conn->prepare("
        INSERT INTO final_defense_annotation_replies (
            annotation_id,
            user_id,
            reply_content,
            reply_user_role
...
function fetch_final_defense_annotation_replies(mysqli $conn, int $annotation_id): array
{
    $stmt = $conn->prepare("
        SELECT
            r.reply_id,
        ...
        FROM final_defense_annotation_replies r
...
function get_final_defense_annotation_statistics(mysqli $conn, int $submission_id): array
{
    $stmt = $conn->prepare("
        SELECT
