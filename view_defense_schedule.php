...
<div class="content">
  <div class="container my-4">
    ...
    <!-- ðŸ”” Animated Defense Tomorrow Banner -->
      ...
      <div class="card-body">
        <?php if (!$paymentVerified): ?>
          <div class="alert alert-info text-center mb-0">
            <i class="bi bi-info-circle"></i> Defense schedule will appear after your payment is verified.
          </div>
...
<script>
// ðŸ•“ Live Countdown + Animated Banner
function updateCountdowns() {
  ...
  document.querySelectorAll('.schedule-card').forEach((card, index) => {
    ...
    if (diffMs > 0) {
      if (daysLeft === 1) {
        showBanner = true;
        bannerText = `âš ï¸ Your defense is tomorrow at <strong>${card.dataset.time}</strong> on <strong>${card.dataset.date}</strong>. Prepare your presentation and requirements.`;
      }
      statusText = `â³ ${daysLeft} day${daysLeft !== 1 ? 's' : ''} left before defense`;
      percent = Math.min(100, Math.max(0, 100 - (daysLeft / totalDays) * 100));
      circleElem.style.stroke = "#16562c";
    } else if (Math.abs(daysLeft) <= 1) {
      statusText = "âœ… Defense is today!";
      percent = 100;
    } else {
      statusText = `ðŸ“… Defense completed ${Math.abs(daysLeft)} day${Math.abs(daysLeft) !== 1 ? 's' : ''} ago`;
      percent = 100;
  ...
  // ðŸ”” Show or hide banner smoothly
